...
namespace DAO
{
    public class Sach_DAO
    {
        ...
        {
            string sql = "select MaSach,DAUSACH.MaDauSach,MaTheLoai,NhaXuatBan,NamXuatBan,DonGiaBan,SoLuongTon,DuocBan from SACH join DAUSACH on SACH.MaDauSach = DAUSACH.MaDauSach";
            return DataAccess.ThucThiQuery(sql);
        }
        //Trả về thông tin sách Full
        public static DataTable SelectThongTinSachFull()
        {
            string sql = "select MaSach,ds.MaDauSach,TacGia,MaTheLoai,NhaXuatBan,NamXuatBan,DonGiaBan,SoLuongTon from DAUSACH ds, CT_TACGIA ct,SACH s where ds.MaDauSach=ct.MaDauSach and ds.MaDauSach=s.MaDauSach and s.DuocBan = 1";
            return DataAccess.ThucThiQuery(sql);
        }
        //Trả về đối tượng SACH giống với tên
        public static string Insert(Sach_DTO s)
        {
            string sql = "insert into SACH values('" + s.MaDauSach + "',N'" + s.NhaXuatBan + "'," + s.NamXuatBan + "," + s.SoLuongTon + "," + s.DonGiaBan + ","+s.DuocBan + ")";
        ...
        //Nếu sách được bán hay không?
        public static string DuocBan(Sach_DTO s)
        {
            string cmd = "update Sach set DuocBan = " + s.DuocBan + " where MaSach = " + s.MaSach;
            return DataAccess.ThucThiNonQuery(cmd);
        }
    }
}
